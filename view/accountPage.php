...
<body><main><h1>Account settings</h1><?php
if (!empty($message)): ?><div class="message"><?= htmlspecialchars(
    $message,
) ?></div><?php endif;
...
?>
<form method="post"><h2>Profile information</h2><label>Username<input name="username" value="<?= htmlspecialchars(
    $_SESSION["username"],
) ?>" required></label><label>Email<input name="email" type="email" value="<?= htmlspecialchars(
    $_SESSION["email"] ?? "",
) ?>"></label><button name="profile">Save profile</button></form>
