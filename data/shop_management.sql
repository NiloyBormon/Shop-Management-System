...
CREATE TABLE IF NOT EXISTS shops (
    ...
    status ENUM('pending', 'approved', 'suspended', 'deleted') NOT NULL DEFAULT 'pending',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    KEY idx_shops_owner_id (owner_id),
    KEY idx_shops_status (status),
    CONSTRAINT fk_shops_owner FOREIGN KEY (owner_id) REFERENCES accounts (id)
        ON UPDATE CASCADE ON DELETE SET NULL
) ENGINE=InnoDB;

INSERT IGNORE INTO shops (id, name, owner_id, status)
    SELECT tenant_id, tenant_name, id, 'approved'
    FROM accounts
...
CREATE TABLE IF NOT EXISTS shop_members (
    ...
    CONSTRAINT fk_members_shop FOREIGN KEY (shop_id) REFERENCES shops (id)
...
INSERT IGNORE INTO shop_members (shop_id, account_id, member_role)
    ...
    FROM accounts
    WHERE tenant_id IS NOT NULL AND role = 'shop_owner';
...
CREATE TABLE IF NOT EXISTS subscriptions (
    ...
    CONSTRAINT fk_subscription_shop FOREIGN KEY (shop_id) REFERENCES shops (id)
        ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

INSERT IGNORE INTO subscriptions (shop_id, plan, payment_status)
    SELECT id, 'free', 'unpaid' FROM shops;
...
CREATE TABLE IF NOT EXISTS products (
    ...
    CONSTRAINT fk_products_account_tenant
...
CREATE TABLE IF NOT EXISTS sales (
    ...
    CONSTRAINT fk_sales_shop FOREIGN KEY (shop_id) REFERENCES shops (id)
        ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_sales_account FOREIGN KEY (account_id) REFERENCES accounts (id)
        ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;
